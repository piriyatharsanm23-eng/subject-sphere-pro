...
import {
  ArrowRight, CalendarClock, Download, FileText, MessageSquare, Star,
} from "lucide-react";
...
function Body({ semesterId }: { semesterId: string }) {
  const stats = useQuery({
    ...
    queryFn: async () => {
      const [m, d, r, f, dl] = await Promise.all([
        ...
        supabase.from("materials").select("download_count").eq("semester_id", semesterId),
      ]);
      const downloads = (dl.data ?? []).reduce((sum, row) => sum + (row.download_count ?? 0), 0);
      return {
        materials: m.count ?? 0,
        deadlines: d.count ?? 0,
        requests: r.count ?? 0,
  ...
  const items = [
    ...
    { label: "Total downloads", value: stats.data?.downloads, icon: Download, color: "text-emerald-400" },
  ];
